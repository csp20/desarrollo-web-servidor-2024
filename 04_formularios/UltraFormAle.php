...
<body>
    <div class="container">
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            //echo "<p>$fecha_actual</p>";

            if($tmp_dni == '') {
                $err_dni = "El DNI es obligatorio";
            } else {
                // strtoupper convierte todo a mayusculas 
            ...
            if($tmp_nombre == '') {
                ...
                if(strlen($tmp_nombre) < 2 || strlen($tmp_nombre) > 40) {
                    $err_nombre = "El nombre debe tener entre 2 y 40 letras";
                } else {
            ...
            if($tmp_apellidos == '') {
                ...
                if(strlen($tmp_apellidos) <= 2 || strlen($tmp_apellidos) > 40) {
                    $err_apellidos = "El apellido debe tener entre 3 y 40 letras";
                } else {
                    //  letras, espacios en blanco y tildes
                    // el patron con todos los caracteres para controlar q se escirbe un nombre
                    $patron = "/^[a-zA-Z áéíóúÁÉÍÓÚñÑüÜ]+$/";
                    // si el apellido no cumple con el patron, error
        ...
        <?php
        if(isset($dni) && isset($correo) && isset($usuario) && isset($nombre) && isset($apellidos) && isset($fecha_actual)) { ?>
            <h1><?php echo $dni ?></h1>
            <h1><?php echo $correo ?></h1>
            <h1><?php echo $usuario ?></h1>
            <h1><?php echo $nombre ?></h1>
            <h1><?php echo $apellidos ?></h1>
            <h1><?php echo $fecha_actual, $tmp_fecha_nacimiento ?></h1>
        <?php } ?>
    </div>
